...
# quantitative-analysis tools. (Went through a dark-theme detour first to fix the mint color
# washing out against the default white plot area - the actual fix was muting the color, not
...
def _add_value_ci(data_metrics_agg: pd.DataFrame, metric_name: str) -> pd.DataFrame:
    ...
    This assumes metric_name is itself a mean (aggregate_metric's agg_func='mean', or
    aggregate_segment_metric's avg_active_metric) - a CI on a sum or other non-mean aggregate
    wouldn't use this formula. Both callers already carry sd_{metric_name}/n_unique_players by
    the time this runs, so this stays a pure attach with no new aggregation.
    """
...
def _attach_test_control_diffs(data_metrics_agg: pd.DataFrame, metric_name: str) -> pd.DataFrame:
    ...
    the input carries sd_{metric_name} and n_unique_players - the same standard-error-of-the-
    difference formula production uses (data_data_science/ab_tests_user_cuped_method/
    ab_trends_functions.py: compute_uplifts_with_mult_ci): combine both variants' SE
    (sqrt(sd_test^2/n_test + sd_control^2/n_control)), then divide by the control mean to land
    on the same % scale as rel_diff_pct. Silently skipped for callers without those columns -
    e.g. aggregate_ratio_metric/aggregate_segment_ratio_metric, matching production's own choice
    not to put a CI on ratio metrics ("no CI for now as dont have covariance").
...
def _total_users_assigned(assignment_data: pd.DataFrame, dts) -> pd.DataFrame:
    """
    Cumulative count of distinct assigned users per variant, evaluated as of each dt - matches
    the dbt model's own definition (ab_dt_segment_metrics.sql: COUNT(DISTINCT IF(dt >=
    assigned_dt, user_id, NULL))), i.e. a fixed-cohort-as-of-that-date denominator, not that
    day's active-user count. Needed so the live pipeline's cumulative charts can be normalized
    the same way production's are - see [[project_abtestmetrics]] for why cumsum'ing an
    already-per-active-user value (the old behavior) doesn't match.

    assignment_data is one row per user with user_id/variant/assigned_dt (abtest_assign.sql's
    output) - independent of activity, unlike data_metrics, so a user assigned but never active
    still counts, same as the dbt model.
    """
...
def aggregate_metric(data_metrics: pd.DataFrame, metric_name: str, agg_func, assignment_data: pd.DataFrame) -> pd.DataFrame:
    ...
    assignment_data (abtest_assign.sql's output - one row per user with variant/assigned_dt) is
    used to attach total_users_assigned, the fixed-cohort denominator plot_metric's cumulative
    charts need - see _total_users_assigned. It doesn't affect the non-cumulative value/rel
    charts at all.
    ...
    can attach CIs. Only meaningful when agg_func='mean' - a CI on a summed or otherwise
    non-mean aggregate wouldn't use this formula.
    """
...
def aggregate_ratio_metric(data_metrics: pd.DataFrame, numerator: str, denominator: str, ratio_name: str) -> pd.DataFrame:
    ...
    Sums numerator and denominator separately per dt/variant, then divides - matching how
    production computes ratio metrics (ab_trends_functions.py's calculate_ratios(), used by
    ab_trends.ipynb) rather than averaging a per-user ratio, which would weight every user's
    ratio equally regardless of how much they actually spent/earned.
    """
    data_metrics_agg = data_metrics.groupby(['dt', 'variant']).agg(
...
def aggregate_segment_metric(data_segment_metrics: pd.DataFrame, metric_name: str) -> pd.DataFrame:
    ...
    Also carries sum_metric (renamed sum_{metric_name}) and total_users_assigned straight
    through - plot_metric's cumulative charts need both to normalize by the fixed-assigned-cohort
    denominator production uses, not that day's active-user count (see aggregate_metric's
    assignment_data param for the live-pipeline equivalent, since this table already has both
    columns baked in by the dbt model).

    sd_active_metric (renamed sd_{metric_name}) is carried through the same way, straight from
    the dbt model's own STDDEV(IF(active=1, metric_value, NULL)) - no extra query needed, it's
    already selected in sql/ab_dt_segment_metrics.sql. Combined with n_unique_players via
    _add_value_ci, this is what lets the static path show a CI band with zero extra BigQuery cost.
    ...
    data_metrics_agg = df.rename(columns={
        ...
        'sum_metric': f'sum_{metric_name}',
        'sd_active_metric': f'sd_{metric_name}',
        'dau_assigned': 'n_unique_players',
    })[['dt', 'variant', metric_name, f'sum_{metric_name}', f'sd_{metric_name}', 'n_unique_players', 'total_users_assigned']].copy()
...
def aggregate_segment_ratio_metric(data_segment_metrics: pd.DataFrame, numerator: str, denominator: str, ratio_name: str) -> pd.DataFrame:
    """
    Ratio-metric equivalent of aggregate_segment_metric() - the STATIC counterpart to
    aggregate_ratio_metric(), built from the pre-aggregated ab_dt_segment_metrics table instead
    of raw per-user rows.

    Pivots numerator/denominator's sum_metric (already a sum across active users for that
    dt/variant, per the dbt model's UNPIVOT) wide and divides - matching both
    aggregate_ratio_metric()'s sum/sum convention and production's own calculate_ratios()
    (ab_trends_functions.py), which pivots this same table's sum_metric column the same way.

    dau_assigned is identical across every metric row for a given dt/variant (it's a property of
    the dt/variant/segment cut, not of the metric - the UNPIVOT just repeats it), so taking it
    from either numerator's or denominator's rows is safe. n_unique_payers is NaN, same reason as
    aggregate_segment_metric() - not available at this grain.
    """
    df = data_segment_metrics[data_segment_metrics['metric'].isin([numerator, denominator])]

    data_metrics_agg = df.pivot_table(
        index=['dt', 'variant'], columns='metric', values='sum_metric'
    ).reset_index()
    dau_assigned = df.groupby(['dt', 'variant'], as_index=False)['dau_assigned'].max()
    data_metrics_agg = data_metrics_agg.merge(dau_assigned, on=['dt', 'variant']).rename(
        columns={'dau_assigned': 'n_unique_players'}
    )
    data_metrics_agg = data_metrics_agg[data_metrics_agg['n_unique_players'] > 0]
...
def _build_cumulative_value(data_metrics_agg: pd.DataFrame, metric_name: str, test_start_date: str) -> pd.DataFrame:
    ...
    cumulative_{metric_name} = cumsum(sum_{metric_name}) / total_users_assigned - matches
    production's own cumulative definition (ab_trends_functions.py: cumsum(sum_metric) /
    total_users_assigned), a fixed-assigned-cohort denominator. Earlier versions cumsum'd
    metric_name itself (already a per-active-user mean), which divides by that day's active-user
    count instead - a different and much larger-magnitude quantity, confirmed by decoding
    production's actual rendered chart data byte-for-byte against this formula - see
    [[project_abtestmetrics]].
    """
...
def _build_cumulative_diff(data_metrics_agg: pd.DataFrame, metric_name: str, test_start_date: str) -> pd.DataFrame:
    ...
    Built from _build_cumulative_value's own output rather than re-deriving a cumsum here, so
    there's exactly one place that defines what "cumulative" means - a second independent
    definition is exactly how these two used to quietly disagree (see [[project_abtestmetrics]]).
    """
...
def _get_overall_diff(data_metrics_agg: pd.DataFrame, metric_name: str, test_start_date: str):
    ...
    so this is guaranteed to match the cumulative charts' final points exactly, not just
    approximately - total_control in particular must come from the same cumulative_{metric_name}
    definition (cumsum(sum_metric)/total_users_assigned), not a plain sum of the per-active-user
    mean column, or it silently drifts out of sync with overall_abs_diff's basis.
...
def _add_date_markers(fig, test_start_date: str = None, assignment_start_date: str = None, row: int = None, col: int = None) -> None:
    """
...
def _ci_band_traces(df: pd.DataFrame, lower_col: str, upper_col: str) -> list:
    ...
    - callers that don't carry sd/n (e.g. ratio metrics, which production itself doesn't put a CI
    on) get no band, with no special-casing needed at the plot_metric call site.
    """
...
def _plot_single(panel: dict, width: int, height: int, test_start_date: str = None, assignment_start_date: str = None) -> None:
    """Fallback for when only one side of a value/diff pair is toggled on - same look as before
    _plot_paired_row existed, just built from the same panel dict shape for consistency."""
...
def _plot_paired_row(left: dict, right: dict, width: int, height: int, test_start_date: str = None, assignment_start_date: str = None) -> None:
    """
    ...
    _add_date_markers(fig, test_start_date, assignment_start_date, row=1, col=1)
    _add_date_markers(fig, test_start_date, assignment_start_date, row=1, col=2)
...
def _plot_pair(show_left: bool, left: dict, show_right: bool, right: dict, width: int, height: int, test_start_date: str = None, assignment_start_date: str = None) -> None:
    ...
    if show_left and show_right:
        _plot_paired_row(left, right, width, height, test_start_date, assignment_start_date)
    elif show_left:
        _plot_single(left, width, height, test_start_date, assignment_start_date)
    elif show_right:
        _plot_single(right, width, height, test_start_date, assignment_start_date)
...
def plot_metric(data_metrics_agg: pd.DataFrame, metric_name: str, show_overall_diff: bool = True, show_value: bool = True, show_rel: bool = True, show_value_cumulative: bool = True, show_rel_cumulative: bool = True, test_start_date: str = None, assignment_start_date: str = None, width: int = 1200, height: int = 600) -> None:
    ...
    rel_diff_ci_lower/rel_diff_ci_upper columns aggregate_metric/aggregate_segment_metric attach
    - see _attach_test_control_diffs. Ratio metrics (aggregate_ratio_metric/
    aggregate_segment_ratio_metric) don't carry those columns, so their charts render without a
    band, same as production. The two cumulative charts never get a CI band - propagating a valid
    one through a cumulative sum over a growing cohort denominator isn't the same formula as the
    daily case and hasn't been built.
    """
    ...
    if show_value or show_rel:
        ...
        _plot_pair(show_value, value_panel, show_rel, rel_panel, width, height, test_start_date, assignment_start_date)
    ...
    if show_value_cumulative or show_rel_cumulative:
        ...
        _plot_pair(show_value_cumulative, cumulative_value_panel, show_rel_cumulative, cumulative_rel_panel, width, height, test_start_date)
